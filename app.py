    ...
    if request.method == 'POST':
        ...
        if nik:
            if len(nik) != 16 or not nik.isdigit():
                ...
                return redirect(url_for('register'))
            if conn.execute("SELECT id FROM users WHERE nik=?",(nik,)).fetchone():
                flash('NIK sudah digunakan oleh akun lain!','error')
                return redirect(url_for('register'))
        conn.execute("INSERT INTO users (nama,email,password,no_hp,alamat,tipe_akun,nik) VALUES (?,?,?,?,?,?,?)",
                     (nama,email,generate_password_hash(password),no_hp,alamat,tipe_akun,nik or None))
        conn.commit()
...
def switch_role():
    ...
    conn = get_db()
    new_role = 'pemilik' if session.get('tipe_akun') == 'peminjam' else 'peminjam'
    conn.execute("UPDATE users SET tipe_akun=? WHERE id=?",(new_role, session['user_id']))
    conn.commit()
    session['tipe_akun'] = new_role
    flash(f"Berhasil beralih ke mode {'Pemilik Barang 🏪' if new_role == 'pemilik' else 'Peminjam 🛍️'}!", 'success')
    return redirect(request.args.get('next') or url_for('home'))
